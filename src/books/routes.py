...
@book_router.post('/create', status_code=status.HTTP_201_CREATED, response_model=BookModel)
async def create_books(book_data : BookCreateModel,
                       session:AsyncSession = Depends(get_session)
                       ) -> dict: 
...
async def get_book_by_id(book_uid : str, 
                         session:AsyncSession = Depends(get_session)
                         ) -> dict:
